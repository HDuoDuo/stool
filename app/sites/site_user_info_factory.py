...
class SiteUserInfoFactory(object):
    ...
    def build(self, url, site_name, site_cookie=None, ua=None, apikey=None, emulate=None, proxy=False):
        ...
        if emulate and chrome.get_status():
            if not chrome.visit(url=url, ua=ua, apikey=apikey, cookie=site_cookie):
